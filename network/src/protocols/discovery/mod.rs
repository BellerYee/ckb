...
pub struct DiscoveryProtocol<M> {
    ...
    announce_internal: Option<Duration>,
    addr_mgr: M,
}
...
impl<M: AddressManager> ServiceProtocol for DiscoveryProtocol<M> {
    fn init(&mut self, context: &mut ProtocolContext) {
        ...
        context
            .set_service_notify(context.proto_id, ANNOUNCE_CHECK_INTERVAL, 0)
    ...
    fn notify(&mut self, context: &mut ProtocolContext, _token: u64) {
        ...
        let announce_internal = self.announce_internal.unwrap_or(ANNOUNCE_INTERVAL);
        ...
        let announce_list: Vec<_> = self
            ...
            .filter_map(|(id, state)| {
                ...
                // check timer
                state
                    .check_timer(now, announce_internal)
